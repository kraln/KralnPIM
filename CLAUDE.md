...
- Terminal.Gui v2 `TextView` with `ReadOnly = true` does not render (content is invisible). Workaround: use `ReadOnly = false` with `CanFocus = false` to prevent user interaction. Confirmed in `sandbox/reader_test/`
