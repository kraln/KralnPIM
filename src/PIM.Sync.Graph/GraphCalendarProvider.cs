...
public sealed class GraphCalendarProvider : ICalendarProvider
{
    ...
    public async Task<string> CreateEventAsync(CalendarEvent evt, CancellationToken ct)
    {
        ...
        var created = await _client!.Me.Calendars[evt.CalendarId].Events
        ...
        return created?.Id ?? throw new InvalidOperationException("Graph did not return an event id for the created event");
    }
